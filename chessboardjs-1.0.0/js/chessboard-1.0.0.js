...
;(function () {
  ...
  whitePower[0]=100;
  blackPower[0]=100;
  whitePower[1]=getRandomInt(89);
  blackPower[1]=getRandomInt(89);
  whitePower[2]=getRandomInt(49);
  blackPower[2]=getRandomInt(49);
  whitePower[3]=getRandomInt(29);
  blackPower[3]=getRandomInt(29);
  whitePower[4]=getRandomInt(29);
  blackPower[4]=getRandomInt(29);
  whitePower[5]=getRandomInt(9);
  blackPower[5]=getRandomInt(9);
  ...
  function constructor (containerElOrString, config) {
    ...
    function buildPieceHTMLTwo (piece, hidden, id) {
      var html = '<div><img src="' + buildPieceImgSrc(piece) + '" '
      if (isString(id) && id !== '') {
        html += 'id="' + id + '" '
      }
      html += 'alt="" ' +
    ...
    function animateSparePieceToSquare (piece, dest, completeFn) {
      ...
      function onFinishAnimation2 () {
        ...
        $destSquare.append(buildPieceHTMLTwo(piece))
    ...
    //function used only by drawBoard
    function drawPositionInstantTwo () {
      // clear the board
      $board.find('.' + CSS.piece).remove()
      ...
      for (var i in currentPosition) {
        ...
        $('#' + squareElsIds[i]).append(buildPieceHTMLTwo(currentPosition[i]))
      }
    }

    function drawBoard () {
      $board.html(buildBoardHTML(currentOrientation, squareSize, config.showNotation))
      drawPositionInstantTwo()
    ...
    function doesTargetPieceDie(sourceSquareId,targetSquareId) {
      if(Number(sourceSquareId.children("div:last").text()) >= Number(targetSquareId.children("div:last").text()))
      return true
      else
      return false
    }
    ...
    function dropDraggedPieceOnSquare (square) {
      ...
      // update position
      var targetSquareId = $('#' + squareElsIds[square])
      var sourceSquareId = $('#' + squareElsIds[draggedPieceSource])
      var isEmptyTargetSquare = (currentPosition[square] == undefined) ? true:false
      var targetPiece = targetSquareId.children('img').attr('data-piece')
      var squareSize = calculateSquareSize()
      if(!isEmptyTargetSquare){
        if(doesTargetPieceDie(sourceSquareId,targetSquareId)) {
          ...
          if(square != draggedPieceSource){
            targetSquareId.children("div:last").remove();
            if(targetSquareId.children("div").length > 0)
            targetSquareId.children("div:last").after(sourceSquareId.children("div:last").detach())
            else//empty square
            targetSquareId.prepend(sourceSquareId.children("div:last").detach())
          }
        }
        else {
          var newPosition = deepCopy(currentPosition)
          delete newPosition[draggedPieceSource]
          newPosition[square] = targetPiece
          
          //draggedPiece = square
          setCurrentPosition(newPosition)

          if(square != draggedPieceSource){
            sourceSquareId.children("div:last").remove();
          }
        ...
        if(square != draggedPieceSource){
          if($('#' + squareElsIds[square]).children("div").length > 0)
          $('#' + squareElsIds[square]).children("div:last").after($('#' + squareElsIds[draggedPieceSource]).children("div:last").detach())
          else//empty square
          $('#' + squareElsIds[square]).prepend($('#' + squareElsIds[draggedPieceSource]).children("div:last").detach())
        }
      }
    ...
    function stopDraggedPiece (location) {
      ...
      if (isFunction(config.onDrop)) {
        ...
        var targetSquareId = $('#' + squareElsIds[location])
        var sourceSquareId = $('#' + squareElsIds[draggedPieceSource])
        var isEmptyTargetSquare = (currentPosition[location] == undefined) ? true:false
